...
export function PomodoroPanel({ locale, compact = false }: { locale: Locale; compact?: boolean }) {
  ...
  const [taskId, setTaskId] = useState(runtime?.taskId ?? "");
  ...
  const tasks = useMemo(() => state.tasks.filter((task) => task.status !== "completed" && (!subjectId || task.subjectId === subjectId)), [state.tasks, subjectId]);
  ...
  const finish = useCallback((automatic = false) => {
    ...
    const elapsed = automatic ? runtime.durationMinutes : Math.max(1, Math.ceil((runtime.durationMinutes * 60 - currentRemaining) / 60));
    mutate((draft) => {
    ...
    setRemaining(0); setAnnouncement(m.focus.saved); router.refresh();
  ...
  useEffect(() => {
    ...
    const initialTimer = window.setTimeout(() => {
      setSubjectId(runtime.subjectId ?? ""); setTaskId(runtime.taskId ?? ""); setDuration(String(runtime.durationMinutes)); sync();
    }, 0);
  ...
  function start() {
    ...
    mutate((draft) => { draft.pomodoroRuntime = { id: uid("pomodoro"), subjectId: subjectId || undefined, taskId: taskId || undefined, durationMinutes: durationNumber, startedAt: started.toISOString(), targetEndAt: new Date(started.getTime() + seconds * 1000).toISOString(), remainingSeconds: seconds, status: "running" }; });
  ...
  return <Card variant="elevated" className={cn("grid gap-7", !compact && "tablet:grid-cols-[1fr_1.2fr] tablet:items-center")}>
    ...
    <div className="grid gap-4">
      <Field label={m.subject}><select className={fieldClass} value={subjectId} disabled={active} onChange={(e) => { setSubjectId(e.target.value); setTaskId(""); }}>{state.subjects.map((s) => <option key={s.id} value={s.id}>{locale === "en" ? s.nameEn : s.name}</option>)}</select></Field>
      <Field label={m.focus.task}><select className={fieldClass} value={taskId} disabled={active} onChange={(e) => setTaskId(e.target.value)}><option value="">—</option>{tasks.map((t) => <option key={t.id} value={t.id}>{t.title}</option>)}</select></Field>
