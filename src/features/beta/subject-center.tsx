...
export function SubjectCenter({ locale, slug }: { locale: Locale; slug: "psychology" | "politics" }) {
  ...
  const selected = points.find((p) => p.id === pointId && p.chapterId === chapterId);
  const due = state.reviewItems.filter((r) => r.kind === "knowledge" && r.status === "due" && points.some((p) => p.id === r.targetId));
  const masteredPoints = points.filter((point) => point.mastery === "mastered").length;
  ...
  function openPoint(id: string) { const point = points.find((entry) => entry.id === id); if (point) setChapterId(point.chapterId); setPointId(id); setTab("learn"); }
  ...
  return <BetaPage title={title} description={`${m.subjectCenter.learn} · ${chapters.length} ${m.subjectCenter.areas} · ${state.units.filter((unit) => unit.subjectId === subject.id).length} ${m.chapter} · ${masteredPoints} / ${points.length} ${m.subjectCenter.savedPoints} ${m.subjectCenter.mastered}`}>
    ...
    {tab === "learn" && chapters.some((chapter) => chapter.id === chapterId) ? <ChapterWorkspace key={chapterId} locale={locale} chapter={chapters.find((chapter) => chapter.id === chapterId)!} onOpenPoint={openPoint} /> : null}
    ...
    {slug === "politics" ? <CurrentAffairs locale={locale} /> : null}
  </BetaPage>;
}
...
function MindMap({ locale, chapters, points, onOpen }: { locale: Locale; chapters: ReturnType<typeof useBetaData>["state"]["chapters"]; points: ReturnType<typeof useBetaData>["state"]["knowledgePoints"]; onOpen: (id: string) => void }) {
  ...
  return <Card><div className="grid gap-3">{chapters.map((chapter) => { const expanded = open.includes(chapter.id); return <div key={chapter.id} className="rounded-md border border-border bg-surface-muted p-3"><button type="button" aria-expanded={expanded} className="flex min-h-11 w-full items-center justify-between text-left type-label text-primary" onClick={() => setOpen((current) => expanded ? current.filter((id) => id !== chapter.id) : [...current, chapter.id])}><span>{locale === "en" ? chapter.titleEn : chapter.title}</span><span>{expanded ? "−" : "+"}</span></button>{expanded ? <div className="ml-3 mt-2 grid gap-2 border-l border-border-strong pl-4">{points.filter((p) => p.chapterId === chapter.id).map((p) => <button key={p.id} type="button" onClick={() => onOpen(p.id)} className="flex min-h-11 flex-wrap items-center justify-between gap-2 rounded-md bg-surface px-3 text-left type-small text-secondary hover:text-primary"><span>{locale === "en" ? p.titleEn : p.title}</span><span className="flex gap-1">{p.frequency === "high" ? <Badge variant="warm">{locale === "en" ? "Key" : "重点"}</Badge> : null}<Badge variant={p.mastery === "mastered" ? "success" : p.mastery === "reviewing" ? "warm" : "neutral"}>{m.subjectCenter[p.mastery]}</Badge></span></button>)}</div> : null}</div>; })}</div></Card>;
}
